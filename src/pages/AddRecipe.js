...
const AddRecipe = (props) => {
  ...
  let newInput = `input`
  const [ingredientInput, setIngredientInput] = useState([{strIngredient: "", strMeasure: ""}]);
...
const handleAddClick = () => {
  setIngredientInput([...ingredientInput, {strIngredient: "", strMeasure: ""}]);
};

const handleAddRecipe = () => {
  axios.put(`${REACT_APP_SERVER_URL}/api/users/recipe`) {
    name = req.body.name;
  ...
  return (
    <div>
      ...
      <form className='search-form'>
        ...
        <input type="text" id="strMeal" name="strMeal" onChange={handleName} value={name.value}></input>
        ...
        <input type="text" id="strArea" name="strArea" onChange={handleArea}></input>
        <label for="strInstructions">Instructions:</label>
        <input type="text" id="strInstructions" name="strInstructions" onChange={handleInstructions}></input>
        <label for="strMealThumb">Thumbnail Address:</label>
        <input type="text" id="strMealThumb" name="strMealThumb" value="https://static.wikia.nocookie.net/clubpenguin/images/8/85/Place_Setting_icon.png/revision/latest?cb=20171129210847" onChange={handleMealThumb}></input>
        <label for="strTags">Tags</label>
        <input type="text" id="strTags" name="strTags" onChange={handleTags}></input>
        <label for="strYoutube">Instructional Video Address</label>
        <input type="text" id="strYoutube" name="strYoutube" value="https://www.youtube.com/watch?v=iik25wqIuFo" onChange={handleYoutube}></input>
        ...
        <button type="button" onClick={e => handleAddRecipe(e)}>Submit</button>
      </form>
    </div>
  )
}
